...
#PLATFORM=-DPLATFORM_ALLWINNER_H2+
...
function build_lib {
	...
	gcc $1 $PLATFORM -c pm.c $DEBUG_PARAM && \
	gcc $1 $PLATFORM -c pwm.c $DEBUG_PARAM && \
	gcc $1 $PLATFORM -c i2c.c $DEBUG_PARAM && \
	
	cd acp && \
	gcc $1 $PLATFORM -c main.c $DEBUG_PARAM && \
	gcc $1 $PLATFORM -c lck.c $DEBUG_PARAM && \
	cd ../ && \
